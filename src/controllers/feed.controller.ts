...
async function getFeed(req: Request, res: Response) {
  try {
    ...
    const followingList = await Follow.find({ follower: user.id }).select(
      "following"
    );
    ...
    const posts = await getPaginatedPosts(
      { author: { $in: followingIds, $ne: user.id }, is_comment: false },
      page,
      user
    );
...
async function getDiscoveryFeed(req: Request, res: Response) {
  try {
    ...
    const page = parseInt(req.query.page as string) || 1;
    const posts = await getPaginatedPosts(
      { author: { $nin: followingIds, $ne: user.id }, is_comment: false },
      page,
      user
    );
